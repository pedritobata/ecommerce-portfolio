...
import ProductUseCaseImpl, { ProductUseCase } from '@/domain/useCases/productUseCase';
...
const productUsecase: ProductUseCase = new ProductUseCaseImpl(new ProductRepositoryImpl()); 
...
>('homePageProductsTab/fetchProducts', async (_, thunkApi) => {
    ...
    if(products instanceof GenericError) return thunkApi.rejectWithValue(products.message) as unknown as string;
